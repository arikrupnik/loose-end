...
module tang_pocket(length, width, thickness, screw_d, screw_l) {
  // one or both sides can accommodate a screw hole for disassembly;
  // caller passes in `screw_d' and `screw_l' to add the screw hole
  
  // tang cutout
  tang_gap = .3; // .2 tight .3 loose for PLA tangs
  wing_tang(length, width, thickness, screw_d, tang_gap);

  if(!is_undef(screw_d) && !is_undef(screw_l)) {
